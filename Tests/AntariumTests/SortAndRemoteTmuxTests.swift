...
struct SortAndRemoteTmuxTests {
    ...
    func parsePlacesAgentsInPanes() {
        let rows = RemoteTmux.parse(sample, host: "build-box")
    ...
    func parseProducesStableIdentity() {
        let first = RemoteTmux.parse(sample, host: "build-box").map(\.id).sorted()
        let again = RemoteTmux.parse(sample, host: "build-box").map(\.id).sorted()
        #expect(first == again)
    ...
    func parseKeepsOnePaneToOneRow() {
        let forked = """
            ...
            """
        #expect(RemoteTmux.parse(forked, host: "build-box").count == 1)
    }
    ...
    func parseSplitsOnTheFirstMarkerOnly() {
        ...
        let output = """
            ...
            911 910 codex /home/se/.codex/packages/standalone/releases/1.2.3/bin/codex
            """
        let rows = RemoteTmux.parse(output, host: "build-box")
    ...
    func parseWorksWithoutAnExecutableSection() {
        ...
        let output = """
            ...
            \(RemoteTmux.exeSeparator)
            """
        #expect(RemoteTmux.parse(output, host: "build-box").first?.agentID == "codex")
    }
